...
При работі із змінними та міксінами в модулях стилів для компиляції треба підключати @import "../core";

Для автоматичного підключения шрифтів із папки dev/fonts товщина в назві файла має бути поділена через слеш (наприклад, Golos-bold.ttf, Golos-regular.ttf), при запуску gulp в папці dev/styles/partials/ згенерується fonts.scss, якщо файл вже є, для оновлення треба його видалити.

В функції @@include є особливий режим @@loop, котрий приймає другим аргументом массив об\`єктів та повторює елемент стільки разів, скільки об`єктів є в массиві. 